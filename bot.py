    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=5) as client:
            ...
            if response.status_code == 200:
                ...
                print(f"⚠️ Gemini API ответил ошибкой: {response.status_code} - {response.text}")
...
try:
    ...
    logging.warning("ADMIN_CHAT_ID не задан или неверен. Используется заглушка.")
...
GEMINI_API_URL = f"{GEMINI_API_BASE_URL}?key={GEMINI_API_KEY}" # Сборка URL с ключом
...
def create_main_keyboard(user_id):
    ...
    callback_button = KeyboardButton(callback_button_text, request_contact=True)
    ...
    async with httpx.AsyncClient(timeout=20.0) as client:
        for attempt in range(3):
            try:
                ...
                logger.warning(f"Gemini API request failed (Attempt {attempt + 1}): {e}. Details: {error_details}")
                ...
                logger.warning(f"Gemini API request failed (Attempt {attempt + 1}, Connection Error): {e}")
                if attempt < 2:
                    await asyncio.sleep(2 ** attempt) 
                    continue
...
async def select_language(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        logger.error(f"❌ ОШИБКА: Приветственный файл изображения не найден по пути: {photo_path}")
...
async def contact_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        logger.info(f"Callback contact received and admin notification sent from user {user.id} with number {phone_number}")
    except Exception as e:
        logger.error(f"Failed to send admin notification to {ADMIN_CHAT_ID} with contact: {e}")
    ...
    await message.reply_text(
        ...
        reply_markup=create_main_keyboard(user_id) # Возвращаем основную клавиатуру
    )
...
async def content_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    ai_exit_keyboard = create_ai_exit_keyboard(user_id) # Клавиатура с одной кнопкой "🔙 Главное меню"
...
def main() -> None:
    ...
    RU_BUTTONS = [MESSAGES['ru'][k] for k in ['btn_addr', 'btn_callback', 'btn_prep', 'btn_laser']]
    UZ_BUTTONS = [MESSAGES['uz'][k] for k in ['btn_addr', 'btn_callback', 'btn_prep', 'btn_laser']]
    
    # Экранируем все символы в названиях кнопок и объединяем их через '|'
    ALL_MENU_BUTTONS_REGEX = f"^{ '|'.join(re.escape(b) for b in RU_BUTTONS + UZ_BUTTONS) }$"
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            SELECT_LANG: [ 
                ...
                MessageHandler(filters.ALL & ~filters.COMMAND, enforce_lang_selection), 
            ],
            ...
            CONTENT_MENU: [
                ...
                MessageHandler(filters.Regex(EXIT_AI_REGEX) & filters.TEXT, exit_chat_to_menu),
                
                # Главный обработчик 4-х кнопок (Адрес, Звонок, Подготовка, Лазер)
                MessageHandler(filters.Regex(ALL_MENU_BUTTONS_REGEX) & filters.TEXT, content_handler),
                
                # Если введен произвольный текст в меню, он возвращает в меню, чтобы ждать выбора кнопки
                MessageHandler(filters.TEXT & ~filters.COMMAND, content_handler),
                CommandHandler("start", start),
                CommandHandler("help", help_command),
            ],
